pipeline {
    ...
    parameters {
        string(name: 'CLUSTER_NAME', defaultValue: 'balraj-cluster', description: 'Enter your EKS cluster name')
    }
        ...
        stage('Cleanup K8s Resources') {
            steps {
                script {
                    ...
                    sh 'kubectl delete deploy singh-app || true'
                    sh 'kubectl delete svc singh-app || true'
            ...
            steps {
                script {
                    ...
                    for key in $(aws kms list-keys --region us-east-1 --query "Keys[*].KeyId" --output text); do
                        ...
                        aws kms schedule-key-deletion --key-id $key --pending-window-in-days 1 --region us-east-1
                    done
                    '''
                }
            }
