# <span style="color: Yellow;"> End-to-End Kubernetes Observability with ArgoCD, Prometheus, and Grafana on KinD using Terraform</span>
...
## <span style="color: Yellow;">Setup Kubernetes dashboard:
<!-- ```sh
...
### <span style="color: cyan;">Verify Helm Version
```sh
helm version
```
![alt text](image-53.png)

### <span style="color: cyan;">Verify Prometheus charts in Helm,namespace, and services
```sh
...
```
##### <span style="color: Cyan;">Verify the ```prometheus``` in browser.<br>
Now, we will open in browser to access services.<br>
```http://<public IP Address of EC2>:9090``` <br>
...
### <span style="color: yellow;">Configure the Grafana
```sh
kubectl get svc -n monitoring
```
```bash
...
You can change the role as well, as per below screenshot.
...
select the matrix, label filter.. Hit on run query>click save.
