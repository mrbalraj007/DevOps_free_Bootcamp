...
resource "aws_instance" "nexus-svr" {
  # ami                    = "ami-0287a05f0ef0e9d9a"      #change ami id for different region
  ami                    = data.aws_ami.ubuntu.id
  instance_type          = "t2.medium"
  key_name               = "MYLABKEY" #change key name as per your setup
  vpc_security_group_ids = [aws_security_group.nexus-VM-SG.id]
  user_data              = templatefile("./nexus_install.sh", {})
...
resource "aws_security_group" "nexus-VM-SG" {
  name        = "nexus-VM-SG"
...
output "instance_ip" {
  value = aws_instance.nexus-svr.public_ip
}