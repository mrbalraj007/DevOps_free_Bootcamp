...
resource "aws_instance" "web" {
  ...
  user_data = <<-EOF
              #!/bin/bash
              hostnamectl set-hostname ${element(var.instance_details, count.index).name}
              echo "127.0.0.1 ${element(var.instance_details, count.index).name}" >> /etc/hosts
              /bin/bash /${element(var.instance_details, count.index).script}
              EOF
